...
const changeSel = async ({ setError, setTxs, sId, newSel }) => {
  try {
    if (!window.ethereum)
    ...
    const tx = await iotmarket.changeSensorSeller(sId, newSel);
    setTxs([tx]);
  } catch (err) {
    setError(err.message);
  }
};
export default function ChangeSeller({ handleChange, formInputData }) {
        ...
        <main className="mt-4 p-4">
          ...
          <div className="my-3">
            <input
              ...
              name="sens_id"
              className="form-control input input-bordered block w-full focus:ring focus:outline-none"
          ...
          <div className="my-3">
            <input
              type="text"
              onChange={handleChange}
              value={formInputData.newSel}
              name="newSel"
              className="form-control input input-bordered block w-full focus:ring focus:outline-none"
              placeholder="New Sensor Seller Account"
            />
          </div>
        </main>
